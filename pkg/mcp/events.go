		...
		{Tool: WithMeta(
			mcp.NewTool("events_list",
				...
				mcp.WithString("start_time",
					mcp.Description("Start time for event retrieval in RFC3339 format or Unix timestamp. Required if end_time is provided.")),
				mcp.WithString("end_time",
					mcp.Description("End time for event retrieval in RFC3339 format or Unix timestamp. Required if start_time is provided.")),
				mcp.WithString("time_window",
					mcp.Description("Time range from now (e.g., '1h', '24h', '7d') — alternative to start_time/end_time.")),
			),
