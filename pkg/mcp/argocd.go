	...
	return []server.ServerTool{
		...
		},
		{
			Tool: mcp.NewTool("argocd_create_application",
				mcp.WithDescription("Create new applications in ArgoCD to establish GitOps workflows for deployment management"),
				mcp.WithString("k8surl", mcp.Description("Kubernetes API server URL"), mcp.Required()),
				mcp.WithString("k8stoken", mcp.Description("Kubernetes API server authentication token"), mcp.Required()),
				mcp.WithString("name",
					mcp.Description("The name of the application"),
		...
		},
		{
			Tool: mcp.NewTool("argocd_delete_application",
				mcp.WithDescription("Remove applications from ArgoCD management while following propagation policies"),
				mcp.WithString("k8surl", mcp.Description("Kubernetes API server URL"), mcp.Required()),
				mcp.WithString("k8stoken", mcp.Description("Kubernetes API server authentication token"), mcp.Required()),
				mcp.WithString("name",
		...
		},
		{
			Tool: mcp.NewTool("argocd_get_resource_actions",
				...
				mcp.WithString("resource_ref",
					mcp.Description("The resource reference in the format of a map with keys 'name', 'namespace', 'kind' (optional), 'group' (optional), 'version' (optional)"),
				),
			),
		...
		},
		{
			Tool: mcp.NewTool("argocd_run_resource_action",
				...
				mcp.WithString("action",
					mcp.Description("The action to perform (e.g., 'restart', 'resume', 'suspend', 'retry')"),
				),
			),
