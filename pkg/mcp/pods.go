...
func (s *Server) initPods() []server.ServerTool {
	return []server.ServerTool{
		{Tool: mcp.NewTool("pods_list",
			mcp.WithDescription("List all pods across Kubernetes namespaces to overview application statuses and resource usage. This tool supports cluster-wide issue triage by identifying problematic pods."),
		...
		{Tool: mcp.NewTool("pods_list_in_namespace",
			mcp.WithDescription("List pods within a specific Kubernetes namespace to focus on a particular application domain. This tool isolates issues for targeted debugging within a defined scope."),
			mcp.WithString("k8surl", mcp.Description("Kubernetes API server URL"), mcp.Required()),
			mcp.WithString("k8stoken", mcp.Description("Kubernetes API server authentication token"), mcp.Required()),
		...
		{Tool: mcp.NewTool("pods_get",
			mcp.WithDescription("Retrieve detailed information about a specific Kubernetes pod, including configuration and events. This tool diagnoses application-specific issues for debugging and resolution."),
			mcp.WithString("k8surl", mcp.Description("Kubernetes API server URL"), mcp.Required()),
			mcp.WithString("k8stoken", mcp.Description("Kubernetes API server authentication token"), mcp.Required()),
		...
		{Tool: mcp.NewTool("pods_delete",
			mcp.WithDescription("Delete a Kubernetes pod to force its recreation and resolve transient issues. This tool applies configuration changes to restore application functionality during incident resolution."),
			mcp.WithString("k8surl", mcp.Description("Kubernetes API server URL"), mcp.Required()),
			mcp.WithString("k8stoken", mcp.Description("Kubernetes API server authentication token"), mcp.Required()),
		...
		{Tool: mcp.NewTool("pods_exec",
			mcp.WithDescription("Execute commands inside a running Kubernetes pod to perform real-time diagnostics. This tool supports debugging by inspecting processes and application behavior within the pod."),
			mcp.WithString("k8surl", mcp.Description("Kubernetes API server URL"), mcp.Required()),
			mcp.WithString("k8stoken", mcp.Description("Kubernetes API server authentication token"), mcp.Required()),
			mcp.WithString("namespace", mcp.Description("Namespace to get the Pod from")),
		...
		{Tool: mcp.NewTool("pods_log",
			mcp.WithDescription("Retrieve logs from a Kubernetes pod to analyze application output and errors. This tool provides critical insights for debugging and identifying root causes of issues."),
			mcp.WithString("k8surl", mcp.Description("Kubernetes API server URL"), mcp.Required()),
			mcp.WithString("k8stoken", mcp.Description("Kubernetes API server authentication token"), mcp.Required()),
		...
		{Tool: mcp.NewTool("pods_run",
			mcp.WithDescription("Run a new Kubernetes pod with a specified container image to test configurations or deploy temporary fixes. This tool validates solutions during incident analysis."),
			mcp.WithString("k8surl", mcp.Description("Kubernetes API server URL"), mcp.Required()),
			mcp.WithString("k8stoken", mcp.Description("Kubernetes API server authentication token"), mcp.Required()),
