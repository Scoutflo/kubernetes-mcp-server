	...
	return []server.ServerTool{
		{Tool: mcp.NewTool("create_argo_rollout_config",
			...
			mcp.WithString("image",
				mcp.Description("Container image for the rollout"),
				mcp.Required(),
			),
			mcp.WithString("strategy",
				mcp.Description("Rollout strategy (bluegreen, canary, basic)"),
				mcp.Required(),
			),
		...
		{Tool: mcp.NewTool("set_argo_rollout_image",
			mcp.WithDescription("Update container images in rollouts to trigger new deployment cycles with updated application versions"),
			mcp.WithString("k8surl", mcp.Description("Kubernetes API server URL"), mcp.Required()),
			mcp.WithString("k8stoken", mcp.Description("Kubernetes API server authentication token"), mcp.Required()),
			mcp.WithString("name",
				mcp.Description("Name of the rollout"),
			...
			mcp.WithString("container",
				mcp.Description("Name of the container to update"),
				mcp.Required(),
			),
			mcp.WithString("image",
				mcp.Description("New image for the container"),
				mcp.Required(),
			),
