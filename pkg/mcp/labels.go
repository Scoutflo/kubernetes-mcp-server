		...
		{Tool: WithMeta(
			mcp.NewTool("label_resource",
				...
				mcp.WithObject("labels",
					mcp.Description("JSON object of labels to apply as string key-value pairs, e.g. {\"app\": \"nginx\", \"env\": \"production\"}. Keys and values must be strings conforming to Kubernetes label syntax."),
					mcp.Required(),
				),
			),
			map[string]any{
		...
		{Tool: WithMeta(
			mcp.NewTool("annotate_resource",
				...
				mcp.WithObject("annotations",
					mcp.Description("JSON object of annotations to apply as string key-value pairs, e.g. {\"description\": \"main app\", \"owner\": \"team-a\"}. Keys must be valid annotation keys; values are arbitrary strings."),
					mcp.Required(),
				),
			),
			map[string]any{
